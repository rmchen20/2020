...
sorted(range(len(cc.values())), key=lambda i: cc.values()[i])[-5:]
```
...
srun --pty -p test -t 45 --mem 6000 /bin/bash submit.sh
```
...
Running samtools flagstat  on the resulting output_starAligned.out.sam file, there are 5329488 fragments mapped. Running the same unique mapping code as above, there are 1290560 uniquely mapped fragments.

b. If you are getting a different number of mappable fragments between BWA and STAR on the same data, why?

Please provide text answer here.

BWA is not splice aware, which makes a difference if there are introns in the gene.
