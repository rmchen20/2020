...
```{r engine="bash", eval=FALSE}
# your bash command
cut -f 4 'public_MC3.maf' | sort -u | wc -l

cut -f 4 'public_MC3.maf' | sort | uniq -c | sort -nr
```
...
```{r engine="bash", eval=FALSE}
# your bash command
cut -f 3 'public_MC3.maf' | sort | uniq -c | sort -nr | head -5
```
...
I would likely pick TTN. TTN has the most variant classifications in our data, which suggests that changes in TTN would be relatively more biologically significant since it would occur more often.
